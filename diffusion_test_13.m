...
% define   
A_1=0.2;
A_2=0.1;%initial chemical concentration per active agent 

D_1=0.5;
D_2=1.0;% diffusion coefficient
...
source_propagation(A_1,A_2,B_1,B_2,D_1,D_2,t_array,delta_x,min_x,max_x,SAVE_MOVIE);
y_out=0; 
...
function source_propagation(A_1,A_2,B_1,B_2,D_1,D_2,t_array,delta_x,min_x,max_x,SAVE_MOVIE)
...
for t_i=2:length(t_array)
    ...
    curr_t = t;
    curr_c_1 =(A_1/(curr_t^0.5))*exp(-(((x-source_x).^2)+((y-source_y).^2))./(4*D_1*curr_t))+B_1;
    curr_c_2 =(A_2/(curr_t^0.5))*exp(-(((x-sink_x).^2)+((y-sink_y).^2))./(4*D_2*curr_t))+B_2;
    ...
    if (t_i>1) && SAVE_MOVIE==1
        ...
        daspect([1 1 1]); colorbar; caxis([0 A_1+A_2]); hold on;
...
if SAVE_MOVIE==1
    v = VideoWriter(['A',num2str(A_1+A_2),'B',num2str(A_1+A_2),'D',num2str(A_1+A_2),'test.mp4'],'MPEG-4');
    open(v);
    writeVideo(v,F);
    close(v);
end
