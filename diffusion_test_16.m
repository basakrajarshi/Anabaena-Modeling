...
A_1=0.1;
A_2=0.2;
A_3=0.4;%initial chemical concentration per active agent 

D_1=0.5;
D_2=0.7;
D_3=0.9;% diffusion coefficient
...
for t_i=2:length(t_array)
    ...
    for j=1:length(source_x_1)
        curr_t = t;
        curr_c_1_1 =(A_1/(curr_t^0.5))*exp(-(((x-source_x_1(j)).^2)+((y-source_y_1(j)).^2))./(4*D_1*curr_t))+B_1;
    ...
    for k=1:length(source_x_2)
        curr_t = t;
        curr_c_1_2 =(A_2/(curr_t^0.5))*exp(-(((x-source_x_2(k)).^2)+((y-source_y_2(k)).^2))./(4*D_2*curr_t))+B_2;
    ...
    for l=1:length(source_x_3)
        curr_t = t;
        curr_c_1_3 =(A_3/(curr_t^0.5))*exp(-(((x-source_x_3(l)).^2)+((y-source_y_3(l)).^2))./(4*D_3*curr_t))+B_3;
